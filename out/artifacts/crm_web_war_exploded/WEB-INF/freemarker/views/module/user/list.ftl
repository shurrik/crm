...
<#--    <div class="bjui-headBar">
        <ul>
            <li class="left"><button type="button" class="btn btn-success" data-url="${wwwroot}/admin/user/add" data-toggle="navtab" data-id="form" data-icon="plus">新增</button></li>
            <li class="left"><button type="button" class="btn btn-danger" data-url="${wwwroot}/admin/user/delete?ids={#bjui-selected}" data-toggle="doajax" data-confirm-msg="确定要删除选中项吗？" data-icon="remove">删除选中行</button></li>
        </ul>
    </div>-->
        ...
        <tbody>
	        ...
	        <#list pageCtx.rows as row>
	        	<tr data-id="${(row.id)!}">
					...
					<td>${(row.userName)!}</td>	    			
					<td>${(row.realName)!}</td>
					<#--<td password="${(row.password)!}"><@p.subStr content=(row.password)! length=15/></td>	-->    			
					  			
					<td>${(row.mobile)!}</td>	   			
			    			
					<td title="${(row.email)!}"><@p.subStr content=(row.email)! length=15/></td>
					<td title="${(row.depId)!}"><@p.subStr content=(row.depId)! length=15/></td>	    			    			
